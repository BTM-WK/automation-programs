...
def build_email_body(input_json):
    ...
    if featured:
        ...
        sc = featured["scoring"]
        # 점수 필드가 dict일 경우 .score 추출, 숫자면 그대로 사용
        def _safe_score(v):
...
def main():
    ...
    try:
        ...
        wpg_result = auto_push_after_analysis(input_json)
        log.info(f"WPG 전달: {wpg_result.get('pushed', 0)}건 push, {wpg_result.get('skipped', 0)}건 skip")
    except Exception as e:
