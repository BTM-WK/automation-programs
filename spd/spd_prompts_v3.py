...
## ★ 입찰 자격요건 검증 (최우선)
분석 시 가장 먼저 아래 자격요건을 확인하세요. 하나라도 미충족이면 즉시 NO-GO 판정:
1. **지역제한**: "OO 소재 업체", "OO시 등록 업체" 등 → WKMG는 서울 소재이므로 타지역 제한 시 NO-GO
2. **해외거점 요구**: "현지 법인/지사/에이전트 보유" 등 → WKMG 해외 거점 없음, NO-GO
3. **특수자격/면허**: "정보통신공사업", "엔지니어링사업자" 등 전문 면허 요구 → WKMG 미보유 시 NO-GO
4. **실적제한**: "동일 사업 수행 실적 3건 이상" 등 구체적 실적 요구 → WKMG 보유 실적과 대조
5. **기업규모 제한**: "중소기업 한정", "대기업 제외" 등 → WKMG는 중소기업
6. **컨소시엄 제한**: "단독 입찰만 가능", "컨소시엄 불가" 등

과업지시서/제안요청서에 위 자격요건이 명시되어 있으면 반드시 eligibility_check에 기록하고, 미충족 시 총점과 관계없이 NO-GO로 판정하세요."""
...
```json
{
  "eligibility_check": {
    "is_eligible": true,
    "region_restriction": {"found": false, "detail": "지역제한 내용 (없으면 빈 문자열)", "wkmg_eligible": true},
    "overseas_requirement": {"found": false, "detail": "해외거점 요구 내용", "wkmg_eligible": true},
    "special_license": {"found": false, "detail": "특수자격/면허 요구 내용", "wkmg_eligible": true},
    "track_record_requirement": {"found": false, "detail": "실적제한 내용", "wkmg_eligible": true},
    "company_size_restriction": {"found": false, "detail": "기업규모 제한 내용", "wkmg_eligible": true},
    "consortium_restriction": {"found": false, "detail": "컨소시엄 제한 내용", "wkmg_eligible": true},
    "disqualification_reason": "자격 미충족 시 사유 (충족이면 빈 문자열)"
  },
  "basic_info": {
    "title": "공고명",
    "bid_type": "A~H형 중 택1",
...
def build_analysis_prompt_v3(
    ...
    deadline = bid_result.get("deadline", "미확인")
