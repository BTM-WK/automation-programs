...
def _extract_deadline_from_text(texts_extracted: list) -> str:
    """첨부파일 텍스트에서 마감일 추출 (deadline이 공란인 경우 fallback)"""
    import re
    patterns = [
        r'제출[^\n]{0,30}(\d{4})[.\-/년]\s*(\d{1,2})[.\-/월]\s*(\d{1,2})[일]?',
        r'마감[^\n]{0,30}(\d{4})[.\-/년]\s*(\d{1,2})[.\-/월]\s*(\d{1,2})[일]?',
        r'접수[^\n]{0,20}기한[^\n]{0,20}(\d{4})[.\-/년]\s*(\d{1,2})[.\-/월]\s*(\d{1,2})[일]?',
        r'입찰[^\n]{0,30}(\d{4})[.\-/년]\s*(\d{1,2})[.\-/월]\s*(\d{1,2})[일]?',
        r'제안서[^\n]{0,20}제출[^\n]{0,20}(\d{4})[.\-/년]\s*(\d{1,2})[.\-/월]\s*(\d{1,2})[일]?',
    ]
    for text_info in texts_extracted:
        text_file = text_info.get("text_file", "")
...
def process_bid(bid: Dict, config: Dict, dry_run: bool = False) -> Dict:
    ...
    # 마감일이 없으면 첨부파일 텍스트에서 추출 시도
    if not result.get("deadline") and result["texts_extracted"]:
        extracted_deadline = _extract_deadline_from_text(result["texts_extracted"])
        if extracted_deadline:
            result["deadline"] = extracted_deadline
            log.info(f"  📅 마감일 추출: {extracted_deadline} (첨부파일에서)")
